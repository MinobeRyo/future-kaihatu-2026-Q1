...
| 項目 | 内容 |
|---|---|
| 無料枠 | 月 1,000 リクエストまで無料 |
| 課金 | 1,001 件目以降 $1.50 / 1,000 件 |
| 必須条件 | Google Cloud で **Billing（請求先）の有効化**が必要 |
| 現在の状態 | 無料トライアル中（¥47,867 クレジット、90 日間） |

**⚠️ 90 日後（トライアル終了）の挙動**
- `フルアカウントを有効にする` を**自分で押さない限り自動課金されない**
- 押さない場合 → Vision API が停止するのみ（課金なし）
- 押した場合 → 月 1,000 件の無料枠が適用され、超過分のみ課金

**トライアル終了後の選択肢**
1. フルアカウントに移行して月 1,000 件無料枠を使い続ける
2. Gemini API に切り替える（画像を直接渡してOCR＋分類を1回で処理、無料・永続）
