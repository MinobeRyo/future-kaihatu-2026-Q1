...
レシートスキャン
    │
    ├─ 娯楽レシート → 娯楽バフ蓄積（×1.1〜1.5）＋ アイテム獲得
    │                    ※ バフは次の通常スキャン時に消費
    │
    └─ 通常レシート → アイテム獲得（娯楽バフ消費済み）
                         │
                         ▼
                    アイテムをホームで使用
                         │
...
| 行動 | メリット | デメリット |
|---|---|---|
| 健康食を買う | 健康値↑大 → 使用時バフ↑ | 成長値は控えめ |
| ジャンク食品を買う | 成長値↑大（ボーナスあり） | 健康値↓ → 枯れ木リスク |
| 娯楽にお金を使う | 娯楽バフ蓄積 / 精神値↑ | 健康値↓ |
| 日用品を買う | 精神値↑ → 使用時バフ↑ | 成長値は小さい |

### バフの多重構造

成長値の最大効率を出すには**3つのバフを同時に高める**必要がある。

```
① 娯楽バフ（上限 ×1.5）
   └ 娯楽レシートを溜めて次のスキャン時に乗せる

② 健康バフ（上限 ×1.25 ≒ +25%）
   └ 健康値 100 超を維持する（水やり + 健康食）

③ 精神バフ（上限 ×1.25 ≒ +25%）
   └ 精神値 100 超を維持する（日用品 + 定期的なスキャン）
...
**「ジャンクとの付き合い方」**  
ジャンク食品は成長値ボーナスが大きいが健康値を削る。
健康値が 50 以下になると枯れ木状態になり成長が完全停止する。
一方、健康値が高いほどアイテム使用時のバフが乗るため、
ジャンクで成長値を稼ぎながらも健康値を 50 超に保つ管理が求められる。

**「娯楽バフのタイミング管理」**  
娯楽バフは通常スキャン時に 1 回ずつ消費される。
高バフ状態で成長値の大きいアイテムを使えば最大効率になるが、
娯楽は健康値を削るため、バフを溜めすぎると枯れ木になるリスクがある。

**「精神値の自然減少」**  
精神値は 24 時間ごとに −12 減少する。
スキャンをサボると精神値が下がり成長倍率が落ちるため、
定期的な購買行動（≒ 実生活での買い物）がゲームの維持に直結する。
